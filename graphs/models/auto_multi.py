...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes,args):
        ...
        self.auto = torch.nn.Parameter(torch.Tensor([self.args.beta_power]))
    ...
    def forward(self, x,x_raw):
        ...
        power = self.auto
        self.args.beta = self.args.beta + (1-float(self.args.beta)) * (float(1) / self.args.iter_max) ** power[0].cpu().detach().numpy()
...
def Auto_res101(args,num_classes=21, pretrained=True):
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], num_classes,args)
